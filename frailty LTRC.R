...
# save_address - a local address to save interim results. After each bootstrap round the results will be saved to the address provided. The address should direct to a folder, where 5 files will be saved: H012,H013,H023,parameters,convergence. Defaults to 1 (all censored observations are used)
...
# inf_f - inflation factor. If, in order to save computation time, only a sub-sample of the censored observations is used, a factor of (number of sampled obs)/(number of censored obs) should be assigned to the weight of each such observation
...
estimate_all <- function(z12,z13,z23,obs_times1,obs_times2,delta1,delta2,delta3,R=rep(0,length(obs_times1)),RecL=0,marginal_13_haz=NULL,marginal_13_grid=NULL,n_Bootstrap=0,tol=0.001,max_Iter=100,initial_pars=NULL,initial_H=NULL,seed=1,save_address=NULL,name=NULL,check_times=seq(0,max(obs_times1,obs_times2),length.out = 100),inf_f = 1)
{
  ...
  H012res_cox <- H013res_cox <- H023res_cox <- par_res_cox <- NULL
  ...
  for(j in (1:(1+n_Bootstrap)))
  {
    ...
    #initialization:
    if(is.null(initial_pars))  #if no initial parameters were provided, naive estimates are computed
    {
      theta_hat <- 0.1
    ...
    while(conv_cond)
    {
      ...
      #estimating the H0 functions after recL:
      ...
      res <- optim(par=c(theta_hat,g12_hat,g13_hat,g23_hat),Likelihood,method ="L-BFGS-B", gr = Likelihood_grad,lower = c(0.000001,rep(-Inf,sumdim)))
      res_pars <- res$par
...
## example for how to run the estimating function:
